...
CREATE OR ALTER   PROCEDURE [dbo].[weekly_by_ssno]
    @meterid int
AS
BEGIN
    IF OBJECT_ID('tempdb..#temp_week') IS NOT NULL
        DROP TABLE #temp_week;
    ...
    FROM (
        ...
        FROM (
            SELECT 
                ...
                DATEPART(WEEK, c.date) AS week_number
            FROM            
                consumptions c
            INNER JOIN
        ...
        ORDER BY 
            q.date DESC
		OFFSET 0 ROWS FETCH NEXT 12 ROW ONLY
    ) AS tab;
